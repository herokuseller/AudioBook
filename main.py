...
@app.route('/')
def home():
    return "AudioBook Bot is Running!", 200
...
async def pdf_to_text(bot, message):
 try:
           if message.reply_to_message:
                ...
                pdf_reader = PyPDF2.PdfReader(pdf) #pdfReaderObject
                await txt.edit("Getting Number of Pages....")
                num_of_pages = len(pdf_reader.pages) # Number of Pages               
                ...
                with open(f'{message.chat.id}.txt', 'a+') as text_path:   
                  for page in range (0,num_of_pages):              
                      page_no = pdf_reader.pages[page] # Iteration of page number
                      page_content += page_no.extract_text()
